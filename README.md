...
一个基于 Go 的后端服务，用于维护 B 站博主列表（UID 或名称），
周期性拉取新视频并检查可用性，逐步累积“绝版视频库”。
支持博主配置文件动态同步与 HTTP 接口添加。
...
- 前端容器会挂载 `frontend/dist` 静态产物，因此在首次
  `docker compose up -d --build` 前，需要先执行一次 `frontend` 本地构建。
- 当前前端默认以 `API 模式` 启动，会自动请求 `http://localhost:8080`。
- `docker-compose.yml` 已将 `./configs/config.yaml` 以可写方式挂载到容器内
  `/app/config.yaml`，并为 `app` 服务开启了 `restart: unless-stopped`。
- 因此前端设置页保存配置后，后端会在当前容器内重启进程并重新加载最新配
  置；重启窗口内接口可能短暂返回失败，刷新或重新同步即可。
- 后端容器启动时会自动执行 `migrations/` 下的内置迁移链，日志中出现「数据库迁移完成」即可。
...
这些变量定义在项目根目录的 `.env.example`。复制为 `.env` 后，`docker compose` 会自动读取。
...
注意：镜像源是否可用取决于你本机的网络、代理和仓库连通性；
即使仓库内置了一组默认值，也应该保留 `.env` 的快速切换能力。
...
- 未发生内容变化时，后端不会重启。
- 在 Docker Compose 部署下，保存成功依赖 `config.yaml` 可写挂载，以及后
  端进程内重启恢复。
...
- 下载地址：`GET /x/player/playurl`
- 登录状态检查：`GET /x/web-interface/nav`
- 名称解析：`GET /x/web-interface/search/type`
- UID 反查：`GET /x/space/wbi/acc/info`

Cookie 支持：

- `bilibili.cookie`：直接填写完整 Cookie
- `bilibili.sessdata`：只填写 `SESSDATA`，服务会自动拼成 Cookie 头
- 认证检查：`bilibili.auth_check_interval`
- 风控退避：`bilibili.risk_backoff_base`、`bilibili.risk_backoff_max`、`bilibili.risk_backoff_jitter`

更完整的当前实现说明见 `docs/bilibili-adapter.md`。
...
结构说明见 `docs/mysql-schema.md`，实际执行来源见 `migrations/*.sql`。
...
- 风控风险：大量请求可能触发限制，建议调小
  `limits.global_qps` / `limits.per_creator_qps`，
  并结合 `bilibili.risk_backoff_*` 退避配置。
- 配置保存后暂时无法联动：
...
- Docker 构建时访问镜像站被 `127.0.0.1:7890` 之类本地代理拦截：
  - `./scripts/deploy.sh` 与 `powershell -File .\scripts\deploy.ps1`
    会在识别到这类 BuildKit 残留代理错误时，
...
- 当前路线图见 `docs/roadmap.md`。
- 当前待办与优先级讨论入口见 `docs/todo.md`。
- README 不再维护独立的长期路线图清单，避免与实现和专题文档漂移。
